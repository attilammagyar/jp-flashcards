...
cat readings.txt \
    | while read
      do
        ...
        # if [[ "x$x" != "x" ]]; then
        curl \
            "http://api.microsofttranslator.com/v2/http.svc/speak?appId=Tm1Nq59z_7mxMl76wqMXLFPgftsIACx1sZ5DxNYd2gcY*&language=ja-JP&format=audio/wav&options=MaxQuality|$gender&text=$(perl -MURI::Escape -e 'print uri_escape($ARGV[0]);' $text)" \
